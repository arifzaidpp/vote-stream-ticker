...
// // Mock data - this will be replaced with data from the backend
...
// // Organize mock data by party for testing purpose
// const mockParties = [
//   {
//     name: 'Party A',
//     candidates: mockCandidates.filter(c => c.party === 'Party A').map(c => ({
//       id: c.id,
//       name: c.name,
//       position: c.position,
//     })),
...
//   {
//     name: 'Party B',
//     candidates: mockCandidates.filter(c => c.party === 'Party B').map(c => ({
//       id: c.id,
//       name: c.name,
//       position: c.position,
//     })),
...
//     return validParties.reduce((acc: Record<string, typeof mockCandidates>, party: { name: string; candidates: { id: string; name: string; position: string; photo?: string }[] }) => {
...
//                 {(candidates as typeof mockCandidates).map((candidate) => {
//                   const entry = entries.find(e => e.candidateId === candidate.id);
//                   const votes = entry?.votes || 0;
//                   const calculatedVotes = entry?.calculatedVotes !== undefined ? entry.calculatedVotes : (votes * voteValue);
...
//     // Example booth data removed for brevity
//   ]);
...
//   const [parties, setParties] = useState<any[]>(mockParties); // Initialize with mock data
...
//       if (data) {
//         const { boothId, roundId, results } = data;
...
//                     ? { 
//                         ...round,
//                         entries: results.map((result: any) => ({
...
//         const boothName = booths.find(b => b.id === boothId)?.boothNumber || 'selected booth';
//         toast({
//           title: "Vote Count Updated",
//           description: `New votes received for ${boothName}`,
//         });
//       }
//     });
...
//       if (data) {
//         const { boothId, roundId, isPublished, results } = data;
        
//         setBooths(prevBooths => prevBooths.map(booth => {
//           // Check if this is the booth we're looking for
//           if (booth.id !== boothId) return booth;
          
//           return {
//             ...booth,
//             countingRounds: booth.countingRounds.map(round => {
//               // Check if this is the round we're looking for
//               if (round.id !== roundId) return round;
              
//               return { 
//                 ...round,
//                 status: isPublished ? 'PUBLISHED' : 'DRAFT',
//                 timestamp: isPublished ? new Date().toISOString() : round.createdAt,
//                 entries: results ? results.map((result: any) => ({
//                   candidateId: result.candidateId,
//                   votes: result.votes,
//                   calculatedVotes: result.calculatedVotes
//                 })) : round.results
...
//         party.candidates.map((candidate: any) => ({
//           id: candidate.id,
//           name: candidate.name,
//           position: candidate.position,
//           party: party.name,
...
//       const candidatesToUse = allCandidates.length > 0 ? allCandidates : mockCandidates;
      
//       setNewRoundEntries(candidatesToUse.map(candidate => ({
//         candidateId: candidate.id,
//         votes: 0, // Ensure votes is explicitly a number
//         calculatedVotes: 0,
//       })));
//       setVoteValue(1);
//     }
...
//       // Get all candidates from current parties
//       const allCandidates = parties.flatMap(party => 
//         party.candidates.map((candidate: any) => ({
//           id: candidate.id,
//           name: candidate.name,
...
//       const entries = candidatesToUse.map(candidate => {
//         const existingEntry = round.results.find(e => e.candidateId === candidate.id);
...
//           votes: 'votes' in entry ? Number(entry.votes) : ('voteCounts' in entry ? Number(entry.voteCounts) : 0),
//           calculatedVotes: entry.calculatedVotes || 0,
//         }))
//       );
...
//       voteValue,
//       votes: newRoundEntries.map(entry => ({
//         candidateId: entry.candidateId,
//         votes: entry.votes
//       }))
//     };
...
//   }, [selectedBoothId, editingRoundId, newRoundEntries, voteValue, socket, electionId]);
...
//     console.log('selectedBoothId', selectedBoothId);
...
//       electionId,
//       boothId: selectedBoothId,
//       roundNumber: booths.find(b => b.id === selectedBoothId)?.countingRounds.length + 1 || 1,
//       voteValue,
...
//     setSelectedBoothId(null);
//   }, [selectedBoothId, newRoundEntries, voteValue, socket, electionId]);
...
//     // Reset entries with all candidates
//     const allCandidates = parties.flatMap(party => 
//       party.candidates.map((candidate: any) => ({
//         id: candidate.id,
//         name: candidate.name,
...
//     socket.emit('publishCountingRound', { boothId, roundId, electionId }, (response: any) => {
//       if (response && response.success) {
//         toast({
//           title: "Round Published",
...
//           description: `Started counting for ${booths.find(b => b.id === boothId)?.boothNumber || 'selected booth'}`,
//         });
//       } else {
//         toast({
//           title: "Operation Failed",
...
//           description: `Completed counting for ${booth.boothNumber}`,
//         });
//       } else {
//         toast({
//           title: "Operation Failed",
...
//                     <h3 className="font-bold text-lg">{booth.boothNumber}</h3>
//                     <Badge
//                       variant={
...
//                             </Badge>
//                           </div>
//                           {round.createdAt && round.status === "PUBLISHED" && (
//                             <span className="text-sm text-gray-500">
//                               Published: {new Date(round.createdAt).toLocaleString()}
//                             </span>
//                           )}
//                         </CardHeader>
...
//                                       ({parties.find(party => party.id === entry.candidate?.partyId)?.name || 'Unknown Party'}- {entry.candidate?.position})
//                                     </span>
//                                   </div>
//                                   <div className="flex items-center gap-4">
//                                     {/* <span className="text-sm text-gray-500">
...
//                   {booth.status === 'COUNTING' && booth.countingRounds.some(round => round.status === 'DRAFT') && (
...
//                         No live connection. Updates will not be sent to the server until connection is restored.
//                       </p>
//                     </div>
//                   )}
//                 </div>
